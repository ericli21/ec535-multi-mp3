/****************************************************************************
** Meta object code from reading C++ file 'menu1.h'
**
** Created: Sat Apr 27 18:48:51 2019
**      by: The Qt Meta Object Compiler version 61 (Qt 4.5.1)
**
** WARNING! All changes made in this file will be lost!
*****************************************************************************/

#include "menu1.h"
#if !defined(Q_MOC_OUTPUT_REVISION)
#error "The header file 'menu1.h' doesn't include <QObject>."
#elif Q_MOC_OUTPUT_REVISION != 61
#error "This file was generated using the moc from 4.5.1. It"
#error "cannot be used with the include files from this version of Qt."
#error "(The moc has changed too much.)"
#endif

QT_BEGIN_MOC_NAMESPACE
static const uint qt_meta_data_Menu1[] = {

 // content:
       2,       // revision
       0,       // classname
       0,    0, // classinfo
       4,   12, // methods
       0,    0, // properties
       0,    0, // enums/sets
       0,    0, // constructors

 // slots: signature, parameters, type, tag, flags
       7,    6,    6,    6, 0x0a,
      22,    6,    6,    6, 0x0a,
      35,    6,    6,    6, 0x0a,
      57,   52,    6,    6, 0x0a,

       0        // eod
};

static const char qt_meta_stringdata_Menu1[] = {
    "Menu1\0\0handleButton()\0handleSkip()\0"
    "handlePrevious()\0song\0slotTest(QListWidgetItem*)\0"
};

const QMetaObject Menu1::staticMetaObject = {
    { &QWidget::staticMetaObject, qt_meta_stringdata_Menu1,
      qt_meta_data_Menu1, 0 }
};

const QMetaObject *Menu1::metaObject() const
{
    return &staticMetaObject;
}

void *Menu1::qt_metacast(const char *_clname)
{
    if (!_clname) return 0;
    if (!strcmp(_clname, qt_meta_stringdata_Menu1))
        return static_cast<void*>(const_cast< Menu1*>(this));
    return QWidget::qt_metacast(_clname);
}

int Menu1::qt_metacall(QMetaObject::Call _c, int _id, void **_a)
{
    _id = QWidget::qt_metacall(_c, _id, _a);
    if (_id < 0)
        return _id;
    if (_c == QMetaObject::InvokeMetaMethod) {
        switch (_id) {
        case 0: handleButton(); break;
        case 1: handleSkip(); break;
        case 2: handlePrevious(); break;
        case 3: slotTest((*reinterpret_cast< QListWidgetItem*(*)>(_a[1]))); break;
        default: ;
        }
        _id -= 4;
    }
    return _id;
}
QT_END_MOC_NAMESPACE
