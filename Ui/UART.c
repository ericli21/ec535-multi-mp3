...
int get_user_number(int device){
	...
	//ACK_SUCCESS
	if(response[2] == 0x0){
		//return response[0] response[1] convert to int
...
int authenticate_user(int device, int user_id){
	...
	if(user_id < 1){
		...
		if(response[2] == 0x01 || response[2] == 0x02 || response[2] == 0x03){
...
//Open communication with UART sensor, return device file ID
int scanner_open(void){

	//Open communication with UART fingerprint scanner
	struct termios  config;
	int device;

	device = open(DEVICEPORT, O_RDWR | O_NOCTTY | O_NDELAY);
	if(device == -1)
	{
		printf( "Communication with UART FAILED\n" );
		return -1;
	}
	...
	tcsetattr(device,TCSANOW,&config);
